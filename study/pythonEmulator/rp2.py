...
def asm_pio(**kw):
    ...
    def dec(f):
        ...
        # Inject all directive functions (wrap_target, wrap, label, etc.)
        for name in _pio_directives:
            f.__globals__[name] = getattr(emit, name)

        for name in _pio_instructions:
            f.__globals__[name] = getattr(emit, name)

        # Inject all predefined PIO constants (pins, x, y, pindirs, etc.)
        f.__globals__.update(_pio_funcs)
...
def asm_pio_encode(instr, sideset_count, sideset_opt=False):
    ...
    if len(emit.prog[0]) != 1:  # _PROG_DATA
        raise PIOASMError("expecting exactly 1 instruction")
    return emit.prog[0][0]  # _PROG_DATA
...
class PIOASMEmit:
    ...
    def start_pass(self, pass_):
        if pass_ == 1:
            ...
            if self.sideset_count:
                ...
                if self.sideset_opt:
                    self.prog[3] |= 1 << 30  # _PROG_EXECCTRL
                    self.sideset_count += 1
    ...
    def delay(self, delay):
        if self.pass_ > 0:
            ...
            self.prog[0][-1] |= delay << 8  # _PROG_DATA
        return self
    ...
    def side(self, value):
        ...
        if self.pass_ > 0:
            ...
            self.prog[0][-1] |= self.sideset_opt << 12 | value << set_bit  # _PROG_DATA
        return self

    def wrap_target(self):
        self.prog[3] |= self.num_instr << 7  # _PROG_EXECCTRL

    def wrap(self):
        assert self.num_instr
        self.prog[3] |= (self.num_instr - 1) << 12  # _PROG_EXECCTRL
        self.wrap_used = True
    ...
    def word(self, instr, label=None):
        ...
        if self.pass_ > 0:
            ...
            self.prog[0].append(instr | label)  # _PROG_DATA
        return self
    ...
    def get_encoded_instructions(self):
        """Returns the list of encoded PIO instructions."""
        return list(self.prog[0])  # _PROG_DATA
